...
## 2025-06-25 - Memory Exhaustion DoS via Eager Response Fetching
**Vulnerability:** Fetching content via `requests.get()` without `stream=True` causes the library to eagerly download the entire response body into memory. If the requested URL points to an extremely large file, this leads to memory exhaustion and application crashes.
**Learning:** Checking `Content-Type` limits or validating responses is ineffective if performed *after* the entire payload has already been pulled into memory.
**Prevention:** Always use `stream=True` to lazy-load HTTP responses, wrap the request in a `with` block (`with session.get(...) as response:`) to ensure safe connection cleanup, check `Content-Type` before consuming the body, and iterate over `response.iter_content` while enforcing a maximum byte limit.
