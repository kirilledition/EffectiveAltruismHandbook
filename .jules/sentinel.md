...
## 2025-03-03 - [Prevent Command Injection in `subprocess.run`]
**Vulnerability:** Command injection via user-supplied paths that start with hyphens.
**Learning:** Tools like `pandoc` executed via `subprocess.run` can misinterpret a file path starting with a hyphen as a command-line flag if not separated carefully, leading to arbitrary flag injection. Using `=` to pass arguments can also cause parsers to trip if the values aren't escaped perfectly.
**Prevention:** Always resolve dynamic/user-supplied file paths to absolute paths, separate flags from their values into distinct list elements (e.g., `["--output", path]` instead of `f"--output={path}"`), and append the `--` separator before positional arguments to explicitly signal the end of options.
